...
class CardPredictor:
    ...
    def predict(self, car_pic):
        ...
        for card_index, card_img in enumerate(card_imgs):
            ...
            # cv2.waitKey(0)
            # grayimg = cv2.cvtColor(card_img, cv2.COLOR_BGR2GRAY)
            # ret3,th3 =  cv2.threshold(grayimg,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
            # print("tesseract: " + pytesseract.image_to_string(th3, lang="chi_sim",config="--dpi 100"))
            # card_imgs[card_index] = th3
            # cv2.imshow("color", th3)
            # cv2.waitKey(0)
            ...
            # print("tesseract: " + pytesseract.image_to_string(card_img, lang="chi_sim"))
            # cv2.imshow("color", card_img)
            # cv2.waitKey(0)
            if limit1 == 0:
                continue
        ...
        for i, color in enumerate(colors):
            ...
            if True:
                ...
                # print("tesseract: " + pytesseract.image_to_string(gray_img, lang="chi_sim"))
                cv2.imshow("test", gray_img)
                # cv2.imwrite("test.png", gray_img)
                # blured_img = cv2.blur(gray_img, (2, 2))
                # cv2.imwrite("test_blur.png", blured_img)
                cv2.waitKey()
                # 黄、绿车牌字符比背景暗、与蓝车牌刚好相反，所以黄、绿车牌需要反向
                if color == "green" or color == "yello":
                    gray_img = cv2.bitwise_not(gray_img)
                ret, gray_img2 = cv2.threshold(
                    gray_img, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
                # gray_img = cv2.addWeighted(gray_img, 1, gray_img2, 0.1, 0.1)

                cv2.imwrite('test.png', gray_img)
                # i = open('images/testa012.jpg', 'rb')
                # i = open('test_12.png', 'rb')
                i = open('test.png', 'rb')
                img = i.read()

                # message = client.licensePlate(img)  # 通用文字识别，每天 50 000 次免费
                ...
                # gray_img = gray_img2
                cv2.imshow("test", gray_img2)
                cv2.waitKey()
                cv2.imshow("test", gray_img)
                cv2.waitKey()
                # 查找水平直方图波峰
                x_histogram = np.sum(gray_img2, axis=1)
                ...
                print("tesseract: " + pytesseract.image_to_string(gray_img2, config="dpi=80"))
                y_histogram = np.sum(gray_img2, axis=0)
                ...
                if point[1] - point[0] < max_wave_dis / 3:
                    point_img = gray_img2[:, point[0]:point[1]]
                ...
                part_cards2 = seperate_card(gray_img2, wave_peaks)
                for card_index, part_card in enumerate(part_cards):

                    # cv2.imshow("card", part_card)
                    # cv2.waitKey()
                    # cv2.imshow("card", part_cards2[card_index])
                    # cv2.waitKey()
                    # 可能是固定车牌的铆钉
                    if np.mean(part_cards2[card_index]) < 255 / 10:
                        # print("a point")
                        continue
                    ...
                    if card_index == 0 and len(part_cards) > 5:
                    ...
                    if charactor == "1" and card_index == len(part_cards) - 1:
...
if __name__ == '__main__':
    c = CardPredictor()
    c.train_svm()
    # r, roi, color = c.predict("images/test0022.jpg")
    # r, _, _ = c.predict("images/testa012.jpg")
    r, roi, color = c.predict("images/processed/fuxingjj01120051111184536591.jpg")
    # r, roi, color = c.predict("fog2.png")
    # r, roi, color = c.predict("images/fuxingjj01120051111184445227.jpg")
    # r, roi, color = c.predict("test001.jpg")
    # r, roi, color = c.predict("test002.jpg")
    # print(sys.argv[1])
